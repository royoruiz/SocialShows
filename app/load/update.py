...
for doc in c_update:
    if doc['show_id'] in ["3683", "31", "1", "10928", "877", "21057", "663", "3918", "25091", "15531", "370", "2433", "1035"]: 
	c_query = c_tvshows.find({"showid": doc['show_id']})
	print 'http://services.tvrage.com/feeds/full_show_info.php?sid=' + doc['show_id']
	if doc['status'] in ["2", "14", "12", "8", "13"]:
		if c_query.count() == 0:
			...
			print "Aqui actualizamos todo menos usuarios"
			print "verificar si ha cambiado algo"
			#continue
	else:
		if c_query.count() == 0:
			response = urllib2.urlopen('http://services.tvrage.com/feeds/full_show_info.php?sid=' + doc['show_id'])
			...
			print unicode(doc['name']).encode("utf-8")
