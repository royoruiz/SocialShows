...
exports.findByName = function(req, res){
    ...
    if (req.query.q_name == "undefined") {
        if (req.query.q_network == "undefined"){
            ...
            TvShows.aggregate(
                { $unwind: "$users" },
                { $group: { _id: {c_id:"$_id", showid: "$showid", name: "$name", network: "$network"}, users_temp: {$addToSet: '$users'}, count: { $sum: 1 }}},
                { $project: {_id : '$_id.c_id', showid: '$_id.showid', name: '$_id.name',network: '$_id.network', users: '$users_temp', num: '$count'}},
                { $limit: 50},
